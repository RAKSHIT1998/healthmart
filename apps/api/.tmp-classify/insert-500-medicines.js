...
function guessMedicineType(item) {
  ...
  if (n.includes('syrup') || n.includes('suspension') || n.includes('solution') || n.includes('emulsion')) return 'syrup';
  ...
  if (n.includes('tablet') || n.includes('pill')) return 'tablet';
  return 'other';
}
...
async function main() {
  ...
  const part1 = require('./new-500-medicines.json');
  const part2 = require('./new-500-medicines-part2.json');
  const allItems = [...part1, ...part2];

  // Dedup within the new list itself (case-insensitive name)
  const seen = new Set();
  const deduped = [];
  ...
  // Manufacturer map (case-insensitive), create missing ones
